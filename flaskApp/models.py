...
class User(db.Model):
    ...
    register_time = db.Column("register_time", DATETIME, default=datetime.datetime.now, nullable=False)
    last_login_time = db.Column("last_login_time", DATETIME, default=datetime.datetime.now, nullable=False, onupdate=func.now())
    status = db.Column("status", INTEGER, default=1, doc="1:in service 2:out of service")
    option = db.Column("option", TEXT(65535), nullable=True, doc="default recognize option;json;lower property")
